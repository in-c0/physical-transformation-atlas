...
`bound`, `max_efficiency`, `applies_to_sources` on constraints; `knowledge_level` on transducers.
...
as written in the source, the scope, the regime, the sources and the year), `environment`,
`summary`, `review`.
...
An absent optional field means *not recorded*, never *false* or *zero*. `search_indexed_through:
