...
| `/api/schema/v0.5.0.json`                            | JSON Schema (draft 2020-12)                                                                                       | none        | `$defs` per export and per record type, generated from the zod definitions                                                                                                                                                                    |
| `/api/schema/v0.4.0.json`, `/api/schema/v0.3.0.json`, `/api/schema/v0.2.0.json` | JSON Schema (draft 2020-12)                                                             | none        | frozen copies of the previous export formats, kept so that older exports' `meta.schema` URLs still resolve (v0.4.0 is the pass-34 contract, the last with the legacy performance keys and the class `circular`; see [migration-v0.4-to-v0.5.md](migration-v0.4-to-v0.5.md))                                                                                                                                    |
...
hot-gas → expansion and hot-gas → gas-dynamic-expansion steps and supplied by a reviewed pathway only with a preceding providing step, an
`auxiliary_requirements` entry or a `regime_establishments` entry (pass 43) explaining how its implementation establishes it), `evidence` (source ids), `status`
...
says a provider needs explaining), `variant_of` (pass 46: the parent pathway of which this record is a narrower architecture on the same exact route — identical steps; the route keeps the parent as its `pathway` and lists the variant's checks under `paths[].variants[]`; an architecture-specific bound lives on the variant's `bounds[]`), `regime_establishments[]` (pass 43: `{ token, kind: implementation-process, component?, explanation, evidence[] }` — a physical process or operating arrangement inside the named implementation that establishes a required regime, such as the combustor / nozzle that sets the pressure drop a combustion MHD generator's expansion needs; not an off-route load (that is an auxiliary) and never a stand-in for a conversion stage the route should carry; the loader admits it only for a token in `regime_provides` whose registry entry says a provider needs explaining, required on the route, with known evidence, and never beside a preceding provider or an auxiliary that establishes the same token), `bounds[]` (pass 42: `{ constraint, evidence[], conditions[], note }` — a typed upper-bound or formula-bound constraint valid only for this exact architecture, refused by the loader when the route already reaches it through a `bounded_by` claim; generic bounds live on phenomena), `knowledge_level`, `performance` (a strict object since pass 42: only
...
structured form: `value_numeric` — or, since pass 44, `value_range: [low, high]` for a datum a source reports only as an aggregate range over its devices or runs (mutually exclusive with `value_numeric`; a range is never reduced to its upper end by any reducer, and a bound passes a range whose upper end is within it, fails one whose lower end exceeds it, and leaves a straddling range unresolved; individual source rows stay individual scalars) — `unit`, `metric` (which bounded quantity it is), `basis` (what the
...
change while their ids stay. Nothing renamed or removed. Within v0.4.0, additive changes dated 21/09/2026 (loop-3 passes 22–24): search runs gain the optional fields `query_compacted`, `expanded_query` (route-search-v1's Google Scholar compaction exception), `segment`, `positions_screened` and `interruption` (the continuation rule); `Source.type` gains `preprint`; `Pathway.status` gains `observed` with the companion field `observed_through`, and CompiledPath gains `composition_observation` (pass 24). Pass 27: `SearchRecord` and the automated bundles gain `composition_terms[]` (`{ term, evidence[], broad? }`, the field's names for a whole composition) and `query_form` gains `route-composite-name` (key `composite-name`), mandatory on every engine once a term is frozen. Pass 26: `Claim` gains `condition_requirements[]`, the graph gains `interfaces[]` (the Interface record) and `ontology.exclusive_groups[]`, `ontology.condition_tags[]` gain `default_scope`, CompiledPath gains `interfaces_recorded[]` and `interface_model_coverage`; the conditions ontology's pairwise `conflicts` shrink to two universal same-region rows with two exclusive groups (material state; temperature regime) replacing the rest; and one enum value is renamed — `magnitude_screen.status` `bounded` → `relation-complete` — which no published revision before rc3c317847bc7 (live for about an hour on 21/09/2026) had ever emitted, because the screen could not reach it until pass 25. Pass 34: the system layer — the graph core gains `systems[]` (the SystemPathway record above), a new endpoint `/api/systems.json` and `counts.systems_named`; nothing else changes. Pass 35: `Pathway.performance.efficiency_typical` is no longer written (every stored value was removed; a reader that still expects it sees it absent). Pass 36: `Pathway.auxiliary_requirements[]` added; regime tokens are validated against the registry served as vocabulary `regime.token` (with the new block `pathway.auxiliary_requirement.kind`). Pass 50 (an API addition beside the exports, outside the contract — no version change): `/api/residuals.json` serves the residual collection tools/audit-closure.mjs derives from the compiled atlas and its audits — every open uncertainty in a current record (search-incomplete, source-read-pending, evidence-single-source, evidence-interpretation-pending, evidence-not-observed, pathway-observed-not-delivered, measurement-boundary-unresolved, measurement-definition-unresolved), each with a deterministic id, its record, why it is unresolved, what exists and what would close it; a live projection that may change with every revision, whereas design/reviews/loop-3/pass-50-closure.md is the immutable snapshot at the close of loop 3. `/api/stats.json` gains `data.projections` (additive within v0.5.0) listing such endpoints. Pass 49 (data and loader rules, no schema change): a search run that obtained no result list — a throttle or a bot check before the first position — is recorded as an attempt (`interruption` set, `result_count_reported: null`, nothing screened) and counts toward no engine's or key's coverage; a blocked run may never carry `result_count_reported: 0`; the segments of one mandatory run add their positions together; the ten default-frontier records carry tonight's attempts and stay `partial`; the frontier search-state audit (design/reviews/loop-3/pass-49-frontier-search-audit.md, tools/audit-frontier-searches.mjs) recomputes every record's obligations from its target. Pass 48 (data only): `output:electricity` loses its "DC power" alias — electrical work is one output whatever the waveform unless a record narrows it; no token, carrier or field for a current form exists, and the electrical output-form / terminal-boundary audit (design/reviews/loop-3/pass-48-electrical-form-audit.md, generated and gated by tools/audit-electrical-form.mjs) records the form an atlas record states for every producer claim, the one conditioning stage on record (rectification) and every electrical datum's numerator boundary, as a report. Pass 47 (data and pages only): a variant pathway's `canonical_url` in `/api/pathways.json` is its own page `/pathway/{slug}` (its `route_id` stays its parent's route; the route page keeps the parent); the generalised stage-omission audit (design/reviews/loop-3/pass-47-stage-omission-audit.md, generated and gated by tools/audit-stage-omission.mjs) is a report, never a classifier — nothing it finds is served. Pass 46 (additive within v0.5.0): `Pathway.variant_of` and `CompiledPath.variants[]` (`{ pathway, checks[] }`); the PEC water-splitting limits typed — Fountaine, Lewerenz & Atwater 2016's ideal single- and dual-junction maxima as upper bounds scoped to water splitting by `requires_basis`, its realistic cases as benchmarks carrying their parameter bundles, and Cheng 2018's 22.8 % for its 1.78 / 1.26 eV pair as the first `Pathway.bounds` entry, on the variant pathway that carries that architecture's data. Pass 45 (data only): every efficiency datum on a pathway was classified by the boundary of its numerator and denominator as read in its source (design/reviews/loop-3/pass-45-dispositions.yaml, gated by tools/audit-stage-route.mjs): a stage figure is `device-stage-efficiency`, a handbook range is not a measurement, a figure for another route leaves the pathway, an unstated boundary is written into `basis`. Pass 44 (additive within v0.5.0): `Measurement.value_range` (`[low, high]`, mutually exclusive with `value_numeric`) for a datum a source reports only as an aggregate range; `metric` gains `device-stage-efficiency`. **v0.5.0** (loop-3 pass 43, 21/09/2026) — one migration boundary for the incompatible cleanup that began in pass 35: the four legacy performance keys (`efficiency_typical`, `power_density`, `efficiency_record`, `theoretical_limit`) are gone and `Pathway.performance` is strict; `Measurement.normalization`, `datum_kind` and `reference_constraint`; `Pathway.bounds[]`, `auxiliary_requirements[]` and `regime_establishments[]`; the system layer (`SystemPathway`, `SystemHandoff`, `/api/systems.json`); `path.frontier_class` `circular` → `same-form`; the expanded regime-token vocabulary. `/api/schema/v0.4.0.json` is frozen from the pass-34 contract (the last additive revision of v0.4.0) and hashed by a regression; every export now declares `version: "0.5.0"` and names `/api/schema/v0.5.0.json`. The field-by-field map is [migration-v0.4-to-v0.5.md](migration-v0.4-to-v0.5.md). The notes that follow record how each change entered, pass by pass. Pass 42: `Pathway.performance.theoretical_limit` is no longer written and the performance object is strict — every limit is a typed constraint entity reached through a `bounded_by` claim on a phenomenon or, for one exact architecture, a `Pathway.bounds[]` entry (`{ constraint, evidence[], conditions[], note }`, upper-bound | formula-bound only); the thermodynamic-bound check merges both through one applicability / metric / basis machinery; four constraint entities (the Planck visible-band fraction, the reversible electrochemical efficiency ΔG/ΔH, the photosynthetic glucose free-energy limit, the physiology-limited photosynthetic maximum as a benchmark) and seven claims were added so that no limit lived only in prose; the measurement parameters `delta_G_kJ_per_mol` and `delta_H_kJ_per_mol` join the registry. All four legacy performance summary keys are now gone. Pass 41 (closing): one enum value is renamed — `path.frontier_class` `circular` → `same-form` ("source and sink carry the same energy form"; the old name and its UI label "round trip" asserted a return to a starting state the compiler never checked — a pumped loop delivering heat is thermal → thermal and no round trip); revisions before rc24d8d754de3's successor emitted `circular`, so a reader filtering on it sees an empty class, not an error. `path.structural_kind` of a route that exactly matches a recorded pathway is `atomic` when the route carries a single conversion phenomenon (42 of the 92 recorded pathways; they were written as `composition`, contradicting the served definition "two or more conversion phenomena"); its evidence still reads from `frontier_class` and `search_status`. `disequilibrium:fluid-flow` now carries `regime_provides: [flow:bulk-fluid-motion]`. Pass 40: `SystemHandoff` gains `through` and the kind `transferred-heat`; a member whose terminal output is not exported must be the from_member of a handoff of that energy form (loader-checked). Pass 39 (closing): `Measurement` gains `datum_kind` (`measured | derived | design-point | simulated | projected`, vocabulary `measurement.datum_kind`; a model-scope datum must carry one of the last four, a measured datum is never model scope), `reference_constraint` (required as `constraint:carnot-limit` for the new metric `carnot-relative-efficiency`), and `theoretical_limit` carries only bounds — benchmarks and achieved fractions of a bound are typed constraints or structured data, never prose; `Pathway.performance.efficiency_record` is no longer written — every record efficiency is a structured physical measurement and pages derive the best recorded value. Pass 37: `Pathway.performance.power_density` is no longer written (every stored value was removed or migrated); `metric` gains `power` (and, in pass 44, `device-stage-efficiency` — one stage's efficiency against the energy delivered to that stage, with the stage and denominator named in `basis`; never a route efficiency); a density metric's unit must state its normalisation and the measurement carries `normalization: { kind: area | volume | mass | length | count, basis }` with the basis drawn from the registry served as vocabulary `measurement.normalization.basis` (`unstated` when a source gives only the dimension) — the same unit never implies a comparable density, so a derived "best recorded power density" groups by metric, unit, kind and basis; an absolute `power` carries none. Otherwise nothing renamed or removed; a v0.4.0 reader that ignores unknown fields and unknown enum values is unaffected, one that validates `type`, `status` or `magnitude_screen.status` strictly must accept the new values.
