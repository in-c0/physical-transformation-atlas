...
try {
  ...
  const key = async (k, mods = 0) => {
    const codes = { ArrowRight: 39, ArrowDown: 40, ArrowLeft: 37, ArrowUp: 38, Enter: 13, Escape: 27, End: 35, Home: 36 };
    await tab.send("Input.dispatchKeyEvent", { type: "keyDown", key: k, code: k, windowsVirtualKeyCode: codes[k], modifiers: mods });
    await tab.send("Input.dispatchKeyEvent", { type: "keyUp", key: k, code: k, windowsVirtualKeyCode: codes[k], modifiers: mods });
  ...
  const state = async () =>
    JSON.parse(
      await tab.eval(`JSON.stringify({
        ...
        probe: document.querySelector('[aria-live]')?.textContent?.replace(/\\s+/g,' ').slice(0, 60)
      })`),
    );
  ...
  await key("End");
  s = await state();
  check("End moves to the last coupling of the family", s.active?.startsWith("D.02 × C.0"), s.active);
  ...
  await key("Home");
  s = await state();
  check("Home moves to the first coupling of the family", /D\.02 × C\.\d\d/.test(s.active ?? ""), s.active);
  await tab.close();
} finally {
  await edge.close();
  server?.close();
