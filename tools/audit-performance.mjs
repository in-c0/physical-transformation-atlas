...
// --pass 35 (default) audits the three efficiency / limit fields; --pass 37 audits power_density with its own dispositions file.
const passArg = process.argv.indexOf("--pass");
const pass = passArg >= 0 ? process.argv[passArg + 1] : "35";
if (pass !== "35" && pass !== "37") { console.error("audit-performance: --pass must be 35 or 37"); process.exit(1); }
const graph = JSON.parse(readFileSync(join(root, "data", "generated", "graph.json"), "utf8"));
const disp = parse(readFileSync(join(root, "design", "reviews", "loop-3", `pass-${pass}-dispositions.yaml`), "utf8"));
const FIELDS = pass === "37" ? ["power_density"] : ["efficiency_typical", "efficiency_record", "theoretical_limit"];
const DISPOSITIONS = new Set([
  ...
  "remove",
  // pass 37 (the second reader's power_density vocabulary)
  "measured-and-projected-mixed",
  "ambiguous-normalisation",
]);
...
const support = (d) => {
  ...
  if (d.disposition === "ambiguous-basis" || d.disposition === "benchmark-not-bound" || d.disposition === "ambiguous-normalisation") return "unverified";
  if (d.disposition === "measured-and-projected-mixed") return "partly";
  return "no";
};
...
const lines = [];
lines.push(`# Pass ${pass} — the legacy ${pass === "37" ? "power_density" : "performance"} audit (generated by tools/audit-performance.mjs --pass ${pass}; do not edit by hand)`);
lines.push("");
lines.push(
...
if (!check) {
  writeFileSync(join(root, "design", "reviews", "loop-3", `pass-${pass}-audit.md`), out);
