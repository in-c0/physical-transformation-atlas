...
export const VOCABULARY: VocabularyEnum[] = [
  ...
  },
  {
    ...
    terms: define(FRONTIER_CLASSES, {
      ...
      candidate:
        "no check fails, the source and the sink do not share an energy form, every constituent is at least demonstrated, and neither claim-level overlap (two or more ordered claims shared with a recorded pathway) nor a phenomena-level variant (two or more phenomena shared as a source-variant or sink-variant of the closest recorded pathway) makes it derived",
      derived:
        "otherwise candidate-quality, but the route shares two or more ordered claims with a recorded pathway (known_pathway_overlap) or two or more phenomena as a source-variant or sink-variant of the closest recorded pathway (closest_known_pathway): it extends, truncates or re-drives something known",
