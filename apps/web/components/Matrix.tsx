...
export function Matrix({ data, density, fill = false, filters = false }: { data: MatrixPayload; density: "home" | "full"; fill?: boolean; filters?: boolean }) {
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      case "ArrowRight":
        next = { r, c: mod ? familyEdge(c, cols, 1) : Math.min(c + 1, cols.length - 1) };
        break;
      case "ArrowLeft":
        next = { r, c: mod ? familyEdge(c, cols, -1) : Math.max(c - 1, 0) };
        break;
      case "ArrowDown":
        next = { r: mod ? familyEdge(r, rows, 1) : Math.min(r + 1, rows.length - 1), c };
        break;
      case "ArrowUp":
        next = { r: mod ? familyEdge(r, rows, -1) : Math.max(r - 1, 0), c };
        break;
      case "Home":
        next = { r, c: mod ? 0 : familyEdge(c, cols, -1) };
        if (!mod) {
          let j = c;
          while (j > 0 && cols[j - 1].family === cols[c].family) j--;
          next = { r, c: j };
        }
        break;
      case "End": {
        let j = c;
        while (j < cols.length - 1 && cols[j + 1].family === cols[c].family) j++;
        next = { r, c: mod ? cols.length - 1 : j };
        break;
      }
      case "Enter":
      case " ":
  ...
  return (
    <div className={`${styles.wrap} ${selected ? styles.withDrawer : ""} ${fill ? styles.fill : ""}`}>
      <div className={styles.main}>
        ...
        <div className={styles.probe} aria-live="polite">
        ...
        <div className={styles.scroller}>
          <div
            ...
            style={{ ["--cols" as string]: cols.length, ["--cell-w" as string]: cellW, ["--cell-h" as string]: cellH }}
            onKeyDown={onKeyDown}
          >
            ...
            <div className={styles.bands} role="presentation">
              {colBands.map((b) => (
