...
function searchDraft(
  ...
  row: MatrixAxis,
  col: MatrixAxis,
): { engine: string; query: string; url: string } {
  const terms = (id: string) => {
    const e = index.entity.get(id);
    const names = [e?.name ?? id, ...(e?.aliases ?? [])]
      .map((x) => x.replace(/\(.*?\)/g, "").trim())
      .filter((x) => x.length > 3 && /^[A-Za-z0-9 \-–]+$/.test(x));
    return (
      "(" +
...
function materialsReport(
  index: AtlasIndex,
  p: CompiledPath,
): { phenomenon: string; materials: string[] }[] {
  return p.phenomena.map((ph) => ({
    ...
    materials: index
      ...
      .filter(
        (c) =>
          c.predicate === "observed_in" &&
          index.entity.get(c.object)?.type === "material",
      )
...
export function CellDrawer({
      ...
      <Drawer label="Cell" title={title} subtitle={subtitle} onClose={onClose}>
        <DrawerSection title="Question">
          ...
          <p className={s.stateSecondary}>
            Loading evidence… The selected coordinate remains visible.
          </p>
        </DrawerSection>
      </Drawer>
    );
  ...
  const direct = cell.direct_claims
    .map((id) => index.claim.get(id)!)
    .filter(Boolean);
  const allBridges = cell.bridge_paths
    .map((id) => index.path.get(id)!)
    .filter(Boolean);
  ...
  const searches = index.graph.searches.filter(
    (x) =>
      x.target.kind === "cell" &&
      x.target.row === row.id &&
      x.target.col === col.id,
  );
  const bridgeConstituent = active
    ? active.constituent_source_ids
        .map((id) => index.source.get(id)!)
        .filter(Boolean)
    : [];
  const bridgeComposition = active
    ? active.composition_source_ids
        .map((id) => index.source.get(id)!)
        .filter(Boolean)
    : [];
  ...
  const copyQuestion = async (format: "md" | "json") => {
    const url =
      typeof window !== "undefined"
        ? `${window.location.origin}/matrix?cell=${row.address}:${col.address}`
        : "";
    const snapshot = {
      url,
      dataset: `r${rev}`,
      ...
      search: searches.length
        ? searches.map(
            (x) =>
              `${x.engine} ${x.date}: ${x.result.replace(/-/g, " ")}${x.reviewed ? " (reviewed)" : " (index only)"}`,
          )
        : "not searched",
    ...
    try {
      await navigator.clipboard.writeText(
        format === "md" ? md : JSON.stringify(snapshot, null, 2),
      );
      setCopied(format);
  ...
  return (
    ...
    >
      ...
      <DrawerSection title="Direct relation" count={direct.length}>
        {direct.length === 0 && !forbidden && (
          <>
            ...
            <p className={s.stateSecondary}>
              This does not imply the relation is impossible.
            </p>
          </>
        )}
        {direct.length === 0 && forbidden && (
          <>
            ...
            <p className={s.stateSecondary}>
              {rowEntity?.name} carries no exergy relative to the reference
              environment; the second law forbids a cyclic device drawing work
              from it. See the constraint claims on{" "}
      ...
      <DrawerSection title="Known bridges" count={bridges.length}>
        {bridges.length === 0 && (
          <>
            ...
            <p className={s.stateSecondary}>
              No qualifying bridge is present in the current claim graph.
            </p>
            <p className={s.stateSecondary} style={{ marginTop: 6 }}>
              <span className="label">Physics screen</span> · Not assessed — no
              recorded composition is available to run the pathway checks on.
            </p>
          </>
        )}
        {excluded > 0 && (
          <p className={s.stateSecondary} style={{ marginBottom: 6 }}>
            ...
            {bridges.length === 1 ? "" : "s"} · {excluded} additional structural
            route{excluded === 1 ? "" : "s"} (one effect plus bookkeeping,
            carrier-expanded copies or energy round trips) excluded from
            candidate status; see the frontier&apos;s Structure toggles.
          </p>
        )}
        <ol className={s.list}>
          {bridges.map((p, i) => {
            ...
            return (
              <li
                key={p.id}
                className={`${s.bridge} ${isActive ? s.bridgeActive : ""}`}
              >
                <button
                  type="button"
                  className={s.bridgeHead}
                  onClick={() => onBridge(i)}
                  aria-pressed={isActive}
                  style={{ width: "100%", textAlign: "left" }}
                >
                  <span className={s.bridgeNum}>
                ...
                <dl className={s.bridgeFacts}>
                  ...
                  <dd>
                    <b>
                      ...
                      <span className={`ev-${p.evidence_status}`}>
                        {EVIDENCE_LABEL[p.evidence_status]}
                      </span>{" "}
                      · floor {p.constituent_floor}
                    </b>
                  </dd>
                    ...
                    <>
                      ...
                      <dd>
                        <b>
                          {ovName}
                          {ov.relation !== "exact"
                            ? ` · ${ov.shared_claims}/${ov.route_claims}`
                            : ""}
                        </b>
                      </dd>
                    </>
      ...
      {active && (
        ...
        >
          ...
          <button
            ...
            aria-controls="cell-materials-panel"
          >
                  ...
                  rep.length >= 2
                    ? rep
                        ...
                        .map((r, i) => ({
                          ...
                          shared: rep[i].materials.filter((m) =>
                            r.materials.includes(m),
                          ),
                          known:
                            rep[i].materials.length > 0 &&
                ...
                return (
                  <>
                    <ul className={s.conditions}>
                      {rep.map((r) => (
                        <li key={r.phenomenon}>
                          {r.phenomenon}:{" "}
                          {r.materials.length
                            ? `observed in ${r.materials.join(", ")}`
                            : "no material observation recorded"}
                        </li>
                      ))}
                    </ul>
                    {shared.length > 0 && (
                      <ul className={s.conditions} style={{ marginTop: 4 }}>
                        {shared.map((x) => (
                          <li key={x.pair}>
                            {x.pair}:{" "}
                            {x.shared.length
                              ? `shared material recorded — ${x.shared.join(", ")}`
                              : x.known
                                ? "no shared material recorded in the atlas"
                                : "insufficient material records"}
                          </li>
                        ))}
                      </ul>
                    )}
                    <p className={s.stateSecondary} style={{ marginTop: 4 }}>
                      ...
                      {active.implied_interface_count === 1 ? "" : "s"} from
                      condition tags. {withMats.length}/{rep.length} phenomena
                      carry material observations. A missing intersection says
                      nothing about compatibility in nature.
                    </p>
                  </>
                );
              })()}
      ...
      <DrawerSection title="Search record" count={searches.length}>
        {searches.length === 0 && (
          <>
            ...
            <p className={s.stateSecondary}>
              No recorded search for this direct relation.
            </p>
          </>
        )}
        <ul className={s.list}>
          {searches.map((x) => (
            <li key={x.id} className={s.claim}>
              ...
              <div className="t-micro secondary">
                {x.reviewed
                  ? `human-reviewed${x.reviewed_by ? ` by ${x.reviewed_by}` : ""}`
                  : "automated index query · not human-reviewed"}
              </div>
              {x.top.length > 0 && (
                <ul className={s.conditions}>
        ...
        {active && (
          <p className={s.stateSecondary} style={{ marginTop: 8 }}>
            Bridge {String(bridges.indexOf(active) + 1).padStart(2, "0")}:{" "}
            {compositionState(active.search_status, active.last_searched).long}
          </p>
        )}
        <div
          className={s.bridgeActions}
        ...
        >
          <button
            ...
            aria-controls="cell-search-panel"
          >
            Prepare search
          </button>
          <button
          ...
          <button
            type="button"
            className={s.linkBtn}
            onClick={() => copyQuestion("json")}
          >
            {copied === "json" ? "Copied" : "Copy JSON"}
          </button>
          {copied === "failed" && (
            <span className="t-micro secondary">clipboard unavailable</span>
          )}
        </div>
        {showSearch && (
          <div id="cell-search-panel" style={{ marginTop: 8 }}>
            <p className={s.stateSecondary}>
              A draft, not a record. Run it, read the hits, and only then add a
              reviewed entry to <code>data/canonical/searches</code>; automated
              hits never change a cell&apos;s status.
            </p>
            <p
              className="t-data"
      ...
      {direct.length > 0 && (
        <DrawerSection
          title="Direct relation evidence"
          count={index.sourcesFor(direct).length}
        >
          <EvidenceList
            sources={index.sourcesFor(direct)}
      ...
      {active && (
        <DrawerSection
          title={`Selected bridge evidence · bridge ${String(bridges.indexOf(active) + 1).padStart(2, "0")}`}
          count={bridgeConstituent.length + bridgeComposition.length}
        >
          ...
          {bridgeComposition.length === 0 ? (
            <p className={s.stateSecondary}>
              No source on record for this exact composition.
            </p>
          ) : (
            <EvidenceList
              sources={bridgeComposition}
              verification={index.graph.source_verification}
            />
          )}
