...
const CORE_CHECKS = new Set(["energy-form-continuity", "conservation", "thermodynamic-bound", "boundary-compatibility"]);
...
function searchDraft(index: AtlasIndex, row: MatrixAxis, col: MatrixAxis): { engine: string; query: string; url: string } {
  const terms = (id: string) => {
    ...
    return "(" + [...new Set(names.map((x) => x.toLowerCase()))].slice(0, 4).map((x) => (x.includes(" ") ? `"${x}"` : x)).join(" OR ") + ")";
  };
    ...
    <Drawer label="Cell" title={title} subtitle={subtitle} onClose={onClose}>
      ...
      <DrawerSection title="Known bridges" count={bridges.length}>
        ...
        {excluded > 0 && (
          <p className={s.stateSecondary} style={{ marginBottom: 6 }}>
            {bridges.length} qualifying composition{bridges.length === 1 ? "" : "s"} · {excluded} additional structural route{excluded === 1 ? "" : "s"} (one effect plus bookkeeping, carrier-expanded copies or energy round trips) excluded from candidate status; see the frontier&apos;s Structure toggles.
          </p>
        )}
        <ol className={s.list}>
          {bridges.map((p, i) => {
      ...
      {active && (
        <DrawerSection title={`Core checks · bridge ${String(bridges.indexOf(active) + 1).padStart(2, "0")}`}>
          <Checksum checks={active.checks.filter((k) => CORE_CHECKS.has(k.id))} claims={active.claims} compact />
          <button type="button" className={s.linkBtn} style={{ marginTop: 8 }} onClick={() => setShowMaterials(!showMaterials)} aria-expanded={showMaterials}>
          ...
          {showMaterials && (
            <div style={{ marginTop: 6 }}>
              {(() => {
                ...
                const shared = rep.length >= 2 ? rep.slice(1).map((r, i) => ({ pair: `${rep[i].phenomenon} → ${r.phenomenon}`, shared: rep[i].materials.filter((m) => r.materials.includes(m)), known: rep[i].materials.length > 0 && r.materials.length > 0 })) : [];
                return (
                  <>
                    ...
                    <p className={s.stateSecondary} style={{ marginTop: 4 }}>
                      {active.implied_interface_count} implied interface{active.implied_interface_count === 1 ? "" : "s"} from condition tags. {withMats.length}/{rep.length} phenomena carry material observations. A missing intersection says nothing about compatibility in nature.
                    </p>
                  </>
                );
              })()}
      ...
      <DrawerSection title="Search record" count={searches.length}>
        ...
        <div className={s.bridgeActions} style={{ marginLeft: 0, marginTop: 8 }}>
          <button type="button" className={s.linkBtn} onClick={() => setShowSearch(!showSearch)} aria-expanded={showSearch}>
            Prepare search
          </button>
      ...
      <DrawerSection title="Coordinates">
        ...
        <p className="t-micro secondary" style={{ marginTop: 8 }}>
          dataset r{rev}
          {excluded > 0 ? ` · ${excluded} route${excluded === 1 ? "" : "s"} classed ${STRUCTURE_LABEL.atomic}, ${STRUCTURE_LABEL["representation-dominated"]} or ${STRUCTURE_LABEL["energy-backtracking"]} are on the frontier under Structure` : ""}
        </p>
      </DrawerSection>
    </Drawer>
  );
