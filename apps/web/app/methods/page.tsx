  ...
  return (
    <main className={styles.main}>
      ...
      <p className={styles.lead}>
        The atlas is a set of reviewed claims about physics, compiled into a graph that can be searched for conversion pathways nobody has demonstrated. This page explains what a claim is, how a pathway inherits a status from its claims, what the seven checks test, and where the language on the site comes from.
      </p>
      ...
      <section className={styles.section} id="ontology">
        ...
        <div className="prose">
          <p>
            The canonical object is not an energy source. It is a physical transformation: a disequilibrium (a temperature difference, a salinity difference, a stressed solid) driving a phenomenon (the Seebeck effect, reverse electrodialysis, piezoelectricity) that hands its energy to a carrier (charge carriers, ionic current, a spin current) and eventually to an output (electrical work, mechanical work, cooling, fuel).
          </p>
          <p>Entities come in these types: system, quantity, disequilibrium, state, interaction, phenomenon, transition, carrier, coupling family, transducer, material, constraint and output. Only four of them can sit in a conversion path — disequilibrium, phenomenon, carrier and output. The rest describe conditions, bounds, materials and devices.</p>
          <p>
            A coupling family (thermoelectric, piezoelectric, osmotic, …) is the column of the matrix. A phenomenon belongs to a family through a <code>member_of</code> claim. A disequilibrium is a row. The cell is whatever the claims say about that pair.
          </p>
        </div>
      </section>
      ...
      <section className={styles.section} id="claims">
        ...
        <div className="prose">
          <p>
            Nothing in the graph is an unquestioned edge. Every relation is stored as a claim with a subject, a predicate, an object, the conditions under which it holds, the sources that support it and a review status. The predicates are:
          </p>
          <p className={styles.mono}>{PREDICATES.join(" · ")}</p>
          <p>
            Four of them — <code>drives</code>, <code>produces</code>, <code>couples_to</code> and <code>converts_into</code> — carry energy from one node to the next and are the only ones the path search follows. Process claims also carry an energy ledger (input form, output form, where the loss goes) and, where a constitutive relation exists, its formula with the unit of its coefficient. In this release {relations} claims carry a relation and {tags} carry machine-checkable condition tags.
          </p>
          <p>A claim's status is one of: established, replicated, demonstrated, reported, theoretically predicted, hypothesised, disputed, contradicted, invalid. A path takes the status of its weakest claim.</p>
        </div>
      </section>
      ...
      <section className={styles.section} id="status">
        ...
        <div className="prose">
          <p>
            Evidence status says how well the physics of a relation is known. Search status says whether anyone has looked for a demonstration of a particular composition. They are independent: the Seebeck effect is established physics, and a five-step composition that uses it may never have been searched. The atlas never turns "we found no paper" into "nobody has tried this". A cell or a path can only say <em>no demonstration found</em> when a reviewed search record exists with that result. Otherwise it says <em>not searched</em>, or <em>index queried, not reviewed</em> when only an automated query has run.
          </p>
      ...
      <section className={styles.section} id="matrix">
        ...
        <div className="prose">
          <p>
            Each cell is a disequilibrium × coupling-family pair. If a <code>drives</code> claim links the row to a phenomenon in the column's family, the cell shows that claim's evidence status (established, demonstrated, theoretical, contradicted, or insufficient when the claim cites nothing). If not, the compiler looks for bridges: enumerated paths from the row that pass through a phenomenon of the column's family. A cell with a bridge whose every constituent is at least demonstrated is a <em>candidate composition</em>. A row whose disequilibrium carries no exergy, and whose constraint claim records that, is <em>forbidden</em>. The remaining cells are <em>searched · none found</em>, <em>index queried</em> or <em>not searched</em>, in that order of what is actually on record.
          </p>
          <p>
            Addresses like D.04 × C.11 are stable dataset addresses: rows and columns are numbered in file order and filtering never renumbers them, so <code>/matrix?cell=D.04:C.11</code> is a permanent link to one scientific question.
          </p>
        </div>
      </section>
      ...
      <section className={styles.section} id="checks">
        ...
        <div className="prose">
          ...
          <dl className={styles.checks}>
            ...
            <dd>The source carries exergy relative to a reference environment, no step is contradicted or invalid, and every step declares its output and its losses. A source with no exergy — heat at uniform temperature, the quantum vacuum — fails here, which is how the second law enters the atlas.</dd>
            <dt>{CHECK_NAME["thermodynamic-bound"]}</dt>
            <dd>Collects the <code>bounded_by</code> constraints of the route's phenomena. When a constraint has a numeric ceiling that applies to the route's source, any recorded efficiency is compared with it.</dd>
            <dt>{CHECK_NAME["dimensional"]}</dt>
            <dd>For each step with a constitutive relation, the SI dimension of the output quantity must equal the dimension of the coefficient times the dimension of the input quantity. Units are parsed from a table in the ontology.</dd>
            <dt>{CHECK_NAME["boundary-compatibility"]}</dt>
            <dd>Condition tags (vacuum, aqueous, cryogenic, above 700 K, ferroelectric, …) are checked against a list of declared conflicts. A conflict inside one step fails. A conflict between two adjacent steps means an interface — a heat exchanger, a window, a shaft — is implied but not recorded, so it is unresolved rather than failed.</dd>
            <dt>{CHECK_NAME["practical-magnitude"]}</dt>
            <dd>Whether a measured efficiency or power density exists on record for this exact composition. For an undemonstrated composition this is unknown, and the site says so instead of inventing a number.</dd>
          </dl>
        </div>
      </section>
      ...
      <section className={styles.section} id="evidence">
        ...
        <div className="prose">
          ...
          <p>
            Search records are the only thing allowed to say "no demonstration found". A record names the engine, the date, the query string, the number of works found and the top hits, and whether a reviewer judged that a qualifying demonstration exists. Automated index queries are stored separately from reviewed searches and are never promoted without review.
          </p>
        </div>
      </section>
      ...
      <section className={styles.section} id="governance">
        ...
        <div className="prose">
          ...
          <p>
            Candidate compositions generated by search are public as compositions of known relations. Newly proposed device concepts with possible patent standing go through a review queue first, following the atlas's publication policy; nothing in this release is in that queue.
          </p>
        </div>
      </section>
      ...
      <section className={styles.section} id="language">
        ...
        <div className="prose">
          ...
          <p>
            The full data is available as JSON under <Link href="/api/graph.json">/api/graph.json</Link>, <Link href="/api/paths.json">/api/paths.json</Link>, <Link href="/api/matrix.json">/api/matrix.json</Link>, <Link href="/api/claims.json">/api/claims.json</Link> (also <Link href="/api/claims.csv">/api/claims.csv</Link>, one row per claim), <Link href="/api/entities.json">/api/entities.json</Link>, <Link href="/api/sources.json">/api/sources.json</Link>, <Link href="/api/pathways.json">/api/pathways.json</Link>, <Link href="/api/coverage.json">/api/coverage.json</Link>, <Link href="/api/stats.json">/api/stats.json</Link> and <Link href="/api/checks.json">/api/checks.json</Link>, which defines the seven checks above and what each of their four results means. Field names and enumerations are documented in the repository under <code>docs/</code>.
          </p>
        </div>
      </section>
    </main>
