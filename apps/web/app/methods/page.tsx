      ...
      <section className={styles.section} id="checks">
        ...
        <div className="prose">
          <p>
            ...
            here so this page cannot drift from the code. The five marked core decide whether a composition is physically coherent — the fifth, driver / regime sufficiency (loop-3 pass 30), asks whether each conversion step gets the regime it needs from its causal source rather than merely a typed edge: pyroelectricity needs a temperature that changes in time, which a static gradient does not supply, so the pyroelectric spelling from a temperature gradient reads unresolved until a route step or a stated condition supplies the change. Requirements and providers are machine-readable tokens on claims, disequilibria and reviewed pathways, every token frozen with its definition in a registry (vocabulary <code>regime.token</code>; an unregistered token fails validation) — since pass 36 a hot-gas → expansion step requires an expansion pressure drop the carrier itself never implies, and a pathway may supply that token only with a preceding providing step, an <code>auxiliary_requirements</code> entry saying how its implementation establishes it (a gas turbine's compressor, driven from its own shaft — a feedback branch no linear route carries, so it is metadata, never a route step) or, since pass 43, a <code>regime_establishments</code> entry for a process inside the implementation itself (the combustor and nozzle that set the pressure drop a combustion MHD generator's expansion needs — while the expansion that makes the flow is a route step, never metadata); a pathway&apos;s own structured temperatures (T_h_K, T_c_K, dT_dt_K_s, T_transition_K, cycle_frequency_Hz, from the registry served as vocabulary <code>measurement.parameter</code>) supply regimes to its exact route only; nothing is inferred from prose. Since pass 19 a check passes only when it has actually decided
            ...
            has been compared with a comparable physical measurement — a model-scope datum (a design point, a simulation) is evaluated and reported as model-consistent or model-inconsistent but never decides the route, just as it supplies no regime (pass 33) — the dimensional check only when every conversion step carries a balanced relation, and the coverage check only when a structured datum
